...
@bot.event
async def on_ready():
    await bot.change_presence(activity=discord.Game(name="FIFA"))
    print(f"{bot.user} ist online")

@bot.slash_command(name = "technick", description = "Bei Technik fragen Tech-Nick fragen!")
async def werbespot(ctx):
    await ctx.respond(random.choice(technick))

@bot.slash_command(name = "choice", description = "Ist diese Nachricht abgenickt oder angepisst von technik 🤔")
async def auswahl(ctx):
    await ctx.respond(random.choice(abgenickt) + " von Technik!")

@bot.slash_command(name = "techfakt", description = "Tech Nick erzählt euch einen zufälligen Tech Fakt!")
async def fakt(ctx):
